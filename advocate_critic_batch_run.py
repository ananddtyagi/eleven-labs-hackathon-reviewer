...
class ProjectDebate:
    ...
    def create_requests(self, project_name: str, description: str):
    ...
    def create_batch_requests(self, projects_dir: str) -> List[Dict]:
        ...
        for filename in os.listdir(projects_dir):
            project_name = filename[:-4][:50]
            project_path = os.path.join(projects_dir, filename)
            description = self.load_project_details(project_path)
            advocate_request, critic_request = self.create_requests(project_name, description)   
